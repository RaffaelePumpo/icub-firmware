...
// C/C++ source code generated on : Thu Jul 10 16:48:59 2025
//
...
void FOCInnerLoop(const boolean_T *rtu_CalibrationDone, const SensorsData
                  *rtu_Sensors, const ActuatorConfiguration
                  ...
                  ZCE_FOCInnerLoop_T *localZCE)
{
  ...
  if (rtu_OuterOutputs->cur_en) {
    ...
    localDW->UnitDelay_DSTATE = rtu_Targets_Inport_5->voltage *
  ...
  if (rtu_OuterOutputs->out_en) {
    ...
    rtb_Product = (localDW->UnitDelay_DSTATE * rtb_SinCos_o2 + rtb_PProdOut_k *
                   rtb_SinCos_o1) * 0.866025388F;

    // Gain: '<S82>/one_by_two' incorporates:
    //   AlgorithmDescriptorDelegate generated from: '<S86>/a16'

    rtb_SinCos_o2 = 0.5F * rtb_Switch_c_idx_0;

    // End of Outputs for SubSystem: '<S85>/Two inputs CRL'

    // Sum: '<S82>/add_c'
    rtb_SinCos_o1 = (0.0F - rtb_SinCos_o2) - rtb_Product;

    // Sum: '<S82>/add_b'
    rtb_SinCos_o2 = rtb_Product - rtb_SinCos_o2;

    // Outputs for Atomic SubSystem: '<S85>/Two inputs CRL'
    // MinMax: '<S1>/Min1' incorporates:
    //   AlgorithmDescriptorDelegate generated from: '<S86>/a16'

    if (rtb_Switch_c_idx_0 <= rtb_SinCos_o2) {
      rtb_PProdOut_k = rtb_Switch_c_idx_0;
    } else {
      rtb_PProdOut_k = rtb_SinCos_o2;
    }

    // End of Outputs for SubSystem: '<S85>/Two inputs CRL'
    if (rtb_PProdOut_k > rtb_SinCos_o1) {
      rtb_PProdOut_k = rtb_SinCos_o1;
    }

    // Gain: '<S1>/Gain3' incorporates:
    //   Product: '<S1>/Divide1'

    rtb_Product = localDW->UnitDelay_DSTATE / rtu_Sensors->driversensors.Vcc *
      100.0F;
    ...
    rtb_Switch_c_idx_0 = (rtb_Switch_c_idx_0 - rtb_PProdOut_k) /
    ...
    rtb_SinCos_o2 = (rtb_SinCos_o2 - rtb_PProdOut_k) /
      rtu_Sensors->driversensors.Vcc * 100.0F + 5.0F;
    ...
    rtb_SinCos_o1 = (rtb_SinCos_o1 - rtb_PProdOut_k) /
      rtu_Sensors->driversensors.Vcc * 100.0F + 5.0F;
  ...
  rty_FOCOutputs->Vabc[1] = rtb_SinCos_o2;
